...
// Google Apps Script Web App URL for Student Leads
// Replace this with your Google Apps Script Web App URL
const STUDENT_LEAD_SCRIPT_URL = process.env.STUDENT_LEAD_SCRIPT_URL 
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!STUDENT_LEAD_SCRIPT_URL || STUDENT_LEAD_SCRIPT_URL.includes('YOUR_SCRIPT_ID')) {
      console.warn('Google Apps Script URL not configured. Form data:', body);
      console.warn('STUDENT_LEAD_SCRIPT_URL value:', STUDENT_LEAD_SCRIPT_URL);
      // Return success even if not configured (for development/testing)
      // In production, you might want to return an error or store locally
    ...
    console.log('Using Google Apps Script URL:', STUDENT_LEAD_SCRIPT_URL);
    ...
    const response = await fetch(STUDENT_LEAD_SCRIPT_URL, {
      method: 'POST',
      headers: {
    ...
    if (response.status === 401 || response.status === 403 || isHtmlError) {
      console.error('❌ Google Sheets API configuration error:', {
        ...
        troubleshooting: [
          '→ Open the URL in your browser: ' + STUDENT_LEAD_SCRIPT_URL,
